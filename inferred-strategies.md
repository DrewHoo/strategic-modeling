...
1. **Identical movers get different live scores (order/state dependent).** In degenerate matrices several strategies collapse to the same move sequence (e.g. A100, GREEDY, and MINIMAX all always-cooperate when the largest cell is `ab`; B100 and BEAT LAST both always-defect in a standard PD). Our stateless engine ties them exactly, but the live game spreads them — by up to ~94 points (game 10: A100 1130 > GREEDY 1066 > MINIMAX 1036). The one robust regularity: **A100 tops its always-cooperate cohort in all 10 captured games**, so `runTournament` breaks ranking ties by ascending lineup index to recover it. This is a heuristic, not the rule — it is *not* a clean "earliest listed wins" gradient: always-defect cohorts violate it and game 7 reverses it (MINIMAX > GREEDY > B100). Net it still helps (winner accuracy 5/10 → 7/10, mean rank error ~1.05 → ~0.93). The most likely cause in the original is the shared mutable strategy objects whose `currentPos` carries across matchups — not reproducible exactly without the source.

2. **BEAT LAST is over-rated in cooperation-heavy / anti-PD matrices.** The engine ranks BEAT LAST higher than the live game in 7 of the 10 fixtures and never lower (worst in games 5, 6, 8, 10 — e.g. game 10 engine ~#1/1111 vs actual #5/1045). A sequential-move hypothesis was tested and rejected (it made fidelity worse). This is an unresolved residual.
